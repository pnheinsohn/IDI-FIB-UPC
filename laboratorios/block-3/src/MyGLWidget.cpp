...
void MyGLWidget::viewTransform() {
  ...
  View = glm::translate(glm::mat4(1.0f), glm::vec3(0, 0, -2 * radiEsc));
...
void MyGLWidget::calculaCapsaModel() {
  ...
  escala = 2.0f / (maxy - miny);
  centrePatr[0] = (minx + maxx) / 2.0f;
  centrePatr[1] = (miny + maxy) / 2.0f;
  centrePatr[2] = (minz + maxz) / 2.0f;
}
...
void MyGLWidget::createBuffersTerraIParet() {
  ...
  // Entrega color a partes no iluminadas para que no sean completamente negras
  glm::vec3 amb(0.2f, 0.0f, 0.2f);
  // Color reflejado por el objeto
  glm::vec3 diff(0.0f, 0.0f, 1.0f);
  // glm::vec3 diff(0.8f, 0.0f, 0.8f);  // Lo deja en color rosado default
  // Color reflejado acorde a ubicacion obs (luz reflejada puede diferir según de donde se observe)
  glm::vec3 spec(0.0f, 0.0f, 1.0f);
  // Nivel de brillante, va de 0 a 128
  float shin = 128.0f;
...
void MyGLWidget::focusLumParams() {
